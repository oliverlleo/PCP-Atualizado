 ...
 * preparar para salvamento no Supabase.
 *
 * @version 2.1.0
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = function (e) {
      try {
        ...
        // Salva os itens no Supabase
        salvarItensNoSupabase(dados, clienteId, tipoProjeto, nomeLista)
          .then(() => {
            resolve({
              sucesso: true,
          ...
          .catch((error) => {
            ...
            reject(new Error(`Erro ao salvar no Supabase: ${error.message}`));
          });
      } catch (error) {
...
/**
 * Salva os itens extraídos no Supabase.
 ...
 */
function salvarItensNoSupabase(itens, clienteId, tipoProjeto, nomeLista) {
  ...
  if (typeof window.supabaseClient === "undefined") {
    return Promise.reject(
      new Error("Supabase não está configurado corretamente.")
    );
  }
