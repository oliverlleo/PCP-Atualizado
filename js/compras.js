    ...
    function tentarCarregarClientes(tentativas = 0, maxTentativas = 5) {
        ...
        if (typeof window.dbRef !== 'undefined' && window.dbRef.clientes) {
            ...
            if (tentativas < maxTentativas) {
                // Aguarda um momento para garantir que o Supabase esteja inicializado
                setTimeout(function() {
                ...
                try {
                    ...
                    if (window.supabaseClient) {
                        // window.dbRef já foi criado no supabase-config.js
                        console.log('Supabase client disponível, dbRef:', window.dbRef);
...
function carregarClientes() {
    ...
    window.dbRef.clientes.once('value')
        .then((snapshot) => {
            console.log('Resposta do Supabase recebida:', snapshot.exists());
    ...
    window.dbRef.projetos.child(clienteId).once('value')
        .then(snapshot => {
            ...
            todosItens.forEach((item, itemIndex) => {
                ...
                // CORREÇÃO: Usar o campo necessidade diretamente do Supabase em vez de calcular
        ...
        projetoRef.once('value')
            .then((snapshot) => {
                ...
                itensSelecionados.forEach(itemSelecionado => {
                    ...
                    // Determinar o caminho correto para o item no Supabase
                    let itemPath;
                    if (item.caminho) {
 ...
 * @param {string} itemPath - Caminho completo do item no Supabase
 * @param {string} clienteId - ID do cliente
