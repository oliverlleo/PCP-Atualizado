...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof window.supabaseClient !== 'undefined') {
        console.log('Supabase detectado. Iniciando config e carregamento...');
        if (!window.dbRef) {
            console.warn('dbRef não definido globalmente. Deveria ter sido criado pelo supabase-config.js.');
            mostrarNotificacao('Erro crítico: Falha ao conectar com Supabase.', 'danger');
            return;
        }
        ...
        mostrarNotificacao('Erro crítico: Supabase não carregado.', 'danger');
    }
});
...
function carregarItensComprados() {
    ...
    window.dbRef.clientes.once('value')
        .then(snapshotClientes => {
            ...
            if (!clientesData) {
                console.warn('Nenhum cliente encontrado no Supabase.');
                inicializarTabelaItens([]);
                carregarEventosCalendario();
...
function confirmarRecebimentoModal() {
    ...
    itensSelecionadosParaRecebimento.forEach(uniqueId => {
        ...
        const itemRef = window.dbRef.projetos; // Adaptação para Supabase
        const caminhoItemNoSupabase = `projetos/${item._fb_clienteId}/${item._fb_tipoProjeto}/listas/${item._fb_nomeLista}/${item._fb_itemKey}`; // Adaptado para Supabase
        ...
        const processarItemPromise = itemRef.once("value").then(snapshot => {
            const itemAtualSupabase = snapshot.val();
            if (!itemAtualSupabase) {
                console.warn(`Item com ID único ${uniqueId} não encontrado no Supabase em ${caminhoItemNoSupabase}.`);
